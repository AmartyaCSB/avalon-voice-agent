-- FIXED COMPLETE DATABASE SETUP FOR AVALON GAME
...
BEGIN
    ...
    FOR room_record IN
        ...
        LEFT JOIN (
            SELECT rp_inner.room_id, COUNT(*) as player_count
            FROM public.room_players rp_inner
            GROUP BY rp_inner.room_id
        ) rp ON gr.id = rp.room_id
        LEFT JOIN (
            SELECT cm_inner.room_id, MAX(cm_inner.created_at) as last_activity
            FROM public.chat_messages cm_inner
            GROUP BY cm_inner.room_id
...
-- Function to get room activity status (for debugging) - FIXED
...
EXCEPTION
    WHEN OTHERS THEN
        -- If there's an error (like table already exists), continue
        RAISE NOTICE 'Realtime publication may already include chat_messages';
END $$;
